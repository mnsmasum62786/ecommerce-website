/**
 * Shared seed logic for Verdant Organic Market.
 *
 ...
 *
 * Consumed by both the CLI seed (`prisma/seed.ts`, `npm run seed`) and a guarded
 * HTTPS setup endpoint (`/api/setup`) so the database can be populated from
 * wherever has network access to it.
 */
import { type PrismaClient, Role, DiscountType } from "@prisma/client";
...
const PRODUCTS: SeedProduct[] = [
  ...
  { name: "Organic Kombucha — Ginger", slug: "organic-kombucha-ginger", category: "beverages", price: 3.99, unit: "16 oz", stock: 90, shortDesc: "Fizzy, gut-friendly ginger kombucha.", description: "Living, raw kombucha fermented with organic green tea and a kick of fresh ginger. Lightly effervescent with billions of probiotics per bottle.", certification: "USDA Organic", tags: ["probiotic", "vegan", "fermented"], image: "https://images.unsplash.com/photo-1517701550927-30cf4ba1dba5?w=800&q=80" },
...
export type SeedResult = { categories: number; products: number; coupons: number; adminEmail: string };
...
export async function runSeed(prisma: PrismaClient): Promise<SeedResult> {
  ...
  await prisma.scriptSettings.upsert({ where: { id: "default" }, update: {}, create: { id: "default" } });
  ...
  await prisma.user.upsert({
    ...
    create: { email: adminEmail, name: process.env.ADMIN_NAME || "Store Admin", role: Role.ADMIN, passwordHash },
  });
  ...
  for (const [i, p] of PRODUCTS.entries()) {
    ...
    await prisma.productImage.create({ data: { productId: product.id, url: p.image, alt: p.name, sortOrder: 0 } });
  }
  ...
  return { categories: CATEGORIES.length, products: PRODUCTS.length, coupons: coupons.length, adminEmail };
}
